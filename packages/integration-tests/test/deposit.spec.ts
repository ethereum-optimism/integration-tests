...
describe('Transactions', () => {
  ...
  it('should allow us to get the l1ToL2TxQueueAddress', async () => {
    ...
    const calldata = await l1ToL2TransactionQueue.interface.encodeFunctionData('enqueueL1ToL2Message', input)
    ...
    const getL1RollupTx = () => postgres.select(`SELECT * FROM l1_rollup_tx WHERE l1_tx_hash = '${txResponse.hash}'`)
    ...
    const getQueueResult = () => postgres.select(`SELECT * FROM geth_submission_queue WHERE l1_tx_hash = '${txResponse.hash}'`)
    const queueResult = await poll(getQueueResult, 10_000)

    queueResult.l1_tx_hash.should.equal(txResponse.hash)
    queueResult.status.should.equal(BatchSubmissionStatus.QUEUED)
  }).timeout(100_000_000)
})
