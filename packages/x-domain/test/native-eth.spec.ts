...
describe('Native ETH Integration Tests', async () => {
  ...
  it('depositTo', async () => {
    ...
    const l1FeePaid = depositReceipts.l1receipt.gasUsed.mul(
      depositReceipts.l1tx.gasPrice
    )
  ...
  it('withdrawTo', async () => {
    ...
    const withdrawalReceipts = await waitForWithdrawalTypeTransaction(
      OVM_ETH.withdrawTo(l1bob.address, withdrawAmount, { gasPrice: 0 }),
      watcher,
      l1Provider,
      l2Provider
    )
  ...
  it('deposit, transfer, withdraw', async () => {
    ...
    const withdrawalReceipts = await waitForWithdrawalTypeTransaction(
      await OVM_ETH.connect(l2bob).withdraw(roundTripAmount, { gasPrice: 0 }),
      watcher,
      l1Provider,
      l2Provider
    )
